...
const E2E_FILES = ['e2e-test.cjs', 'e2e-test-sw.cjs', 'e2e-test-cfg.cjs', 'e2e-test-zh.cjs', 'e2e-test-wizard.cjs', 'e2e-test-ssml.cjs', 'e2e-test-a11y.cjs', 'e2e-test-perf.cjs', 'e2e-test-avail.cjs'];
...
function main() {
  ...
  const pass = e2eTotal >= 51 && unitTotal >= 50 && !versionFail;
  lines.push(`TOTAL: ${total} · 门槛(E2E≥51 且 单元≥50 且 版本一致) ${pass ? 'PASS' : 'FAIL'}`);
