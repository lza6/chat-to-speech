...
| 单元全量 | 8 套件 | **50/50 PASS** |
| 台账门禁 | `node verify-acceptance.cjs --strict` | **TOTAL 101 PASS** + `VERSION: 4.6.2 · 四处一致`（当前真源已升至 4.6.3，见第 14 节）|
...
## 14. v4.6.3 落地台账（2026-09-15）—— E2E 确定性收口（消除跨用例状态串扰）

### 14.1 问题（tag 运行 34877589986 暴露，**同一 SHA 的另一次运行全绿**）

| 项 | 内容 |
|------|------|
| 现象 | v4.6.2 的 tag 运行 shard 2 `T_SSML_1` 失败：「时间线未记录 ssml 解析成功事件」，同时状态栏显示上一步遗留的降级文案 / 朗读完毕文案 |
| 定性 | **本机连跑复现（3 跑 2 败）→ 是测试自身缺陷，不是产品缺陷** |
| 根因 | `e2e-test-ssml.cjs` 4 个用例**共享同一个 page**，并用固定 `waitForTimeout(4500/3500/700ms)` 断言。上一用例的朗读闭环（端点探测 + 1.5/2/3 降级链）迟到完成，把「🔊 离线朗读完毕」写进共享状态栏，污染下一用例断言；且未等应用初始化就 fill/click，行为随启动时序漂移 |

### 14.2 修复（只改测试，未动产品代码）

- 每用例**独立 page**（`ctx.newPage()` + `finally { page.close() }`）。
- **显式就绪等待**：`waitForFunction(() => window.__chattts && window.__chattts.version)`，监听器绑定完成后才交互。
- **轮询断言**替代固定 sleep：`until(fn, ms)`，目标状态出现即通过。
- fill 后**回读输入框值**并断言一致：若未来出现“输入被意外覆盖”，报错信息会直接指认原因，而非隐晦超时。

### 14.3 稳定性证据（真实复跑）

| 验证 | 结果 |
|------|------|
| `e2e-test-ssml.cjs` 连跑 6 次 | **6/6 全绿** |
| shard-2 全套件（cfg/zh/wizard/ssml/perf/avail）连跑 3 轮 | **81/81 全绿**（3 × 27）|
| E2E 全量 9 套件 | **51/51 PASS** |
| 单元全量 8 套件 | **50/50 PASS** |
...
### 14.6 补充（同日）：a11y 套件同型收口（T_A11Y_2 CI 独有失败）

| 项 | 内容 |
|------|------|
| 现象 | main 运行 `34880651510`（纯 CI 配置提交）shard 1 `T_A11Y_2` 失败：「#tts-input 无法获得焦点」；本地连跑 6/6 全绿 → CI 独有 |
| 归因 | 14.2 的非阻塞加载使第三方浮层出现时机改变，弹窗初始化期间短暂抢焦点属**合法行为**，不应记为我方控件缺陷；旧断言为一次性同步判定，无重试也无诊断信息 |
| 修复 | ① 每用例独立 page + 显式等 `__chattts` 就绪（同 14.2 铁则）；② 判定前先 Escape 关闭遮挡弹窗；③ 聚焦轮询最多 3s；④ 失败时输出证据（activeElement/hasFocus/弹窗数/inert 祖先） |
| 验证 | 本地连跑 5/5 + 全量回归 **101/101**；证据化诊断输出已内建，下次失败可一眼归因 |
